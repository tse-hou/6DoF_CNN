...
#         self.random_settings()
    ...
    def random_settings(self):
        self.r_db = random.choice(self.dbs);#print(self.r_db)
        temp_db = self.db[self.r_db];
        self.r_frame = random.choice(temp_db.Frame.unique());#print(self.r_frame)
        self.r_tvs = random.choice(temp_db.loc[(temp_db.Frame == self.r_frame)]["Synthesized.View"].unique());#print(self.r_tvs)
        self.r_data = temp_db.loc[(temp_db.Frame == self.r_frame) & (temp_db['Synthesized.View'] == self.r_tvs)]
        # state transition
        self.state_trans = self.r_data [['p1','p2','p3','CEL','WS.PSNR','theo_time']].copy()
    ...
    def gen_observation(self):
        ...
        frame_idx = np.where(np.array(temp_sv['fn_frames']) == str(self.r_frame))[0][0];#print(frame_idx)
        # print(self.sv[self.r_db]['fn_sv'])
        # print(np.array(self.r_data.filter(regex='^V',axis=1).iloc[0]))
        sort_order = [np.where(temp_sv['fn_sv'] == str(i))[0][0] for i in np.array(self.r_data.filter(regex='^V',axis=1).iloc[0])];#print(sort_order)
        ...
        sort_order = [np.where(np.asarray(temp_sv['c_para'].Name) == ('v'+str(i)))[0][0] for i in np.array(self.r_data.filter(regex='^V',axis=1).iloc[0])];#print(sort_order)
        self.P_src = temp_sv['c_para'].loc[sort_order,'Position1':'Position3'].values;#print(self.sort_cp)
        self.O_src = temp_sv['c_para'].loc[sort_order,'Rotation1':'Rotation3'].values;#print(self.sort_co)
        # get target parameter
        tar_idx = np.where(np.asarray(temp_sv['c_para'].Name) == ('v'+str(self.r_tvs)))[0][0]
        self.P_tar = temp_sv['c_para'].loc[tar_idx,'Position1':'Position3'].values;#print(tar_cp)
        self.O_tar = temp_sv['c_para'].loc[tar_idx,'Rotation1':'Rotation3'].values;#print(tar_co)
    
    def print_training_data_infos(self):
        print("[{0}] frame:{1}, V_tar:{2}, I_src:{3}, D_src:{4}\nP_src:\n{5}\nO_src:\n{6}\nP_tar:\n{7}\nO_tar:\n{8}".format(self.r_db,self.r_frame,self.r_tvs,self.I_src.shape,self.D_src.shape,self.P_src,self.O_src,self.P_tar,self.O_tar))